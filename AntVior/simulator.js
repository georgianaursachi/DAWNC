...
document.querySelector('#sandbox_start').addEventListener('click', function(e) {
    fadeOut(document.querySelector('#config'));
    init();
	removeClass(document.querySelector('#tilesDrawer'), 'hideright');
	document.querySelector(e.currentTarget.nodeName).style.display = 'none';
});

//Config Events

document.querySelector('#initial_population').addEventListener('change', function(e) {
	antFarm.INITIAL_POPULATION = document.getElementById("initial_population").value;
});

document.querySelector('#initial_map_food').addEventListener('change', function(e) {
	antFarm.INITIAL_MAP_FOOD = document.getElementById("initial_map_food").value;
});

document.querySelector('#initial_colony_food').addEventListener('change', function(e) {
	antFarm.INITIAL_COLONY_FOOD = document.getElementById("initial_colony_food").value;
});

document.querySelector('#max_energy').addEventListener('change', function(e) {
	antFarm.MAX_ENERGY = document.getElementById("max_energy").value;
});

document.querySelector('#food_growth_rate').addEventListener('change', function(e) {
	antFarm.FOOD_GROWTH_RATE = document.getElementById("food_growth_rate").value;
});

document.querySelector('#dizziness').addEventListener('change', function(e) {
	antFarm.DIZZINESS = document.getElementById("dizziness").value;
});

document.querySelector('#chance_turn').addEventListener('change', function(e) {
	antFarm.CHANCE_TURN = document.getElementById("chance_turn").value;
});

document.querySelector('#chance_turn_on_trail').addEventListener('change', function(e) {
	antFarm.CHANCE_TURN_ON_TRAIL = document.getElementById("chance_turn_on_trail").value;
});

document.querySelector('#chance_turn_food').addEventListener('change', function(e) {
	antFarm.CHANCE_TURN_FOOD = document.getElementById("chance_turn_food").value;
});
...
function init() {
    document.getElementById("drawer-toggle-label").style.display = 'block';
	...
	var ant = function() {
		...
		if(antCount > maxAnts)
			maxAnts = antCount;
		document.querySelector('#ants').innerHTML = antCount;
		var id = nextAntID++;
		...
		var antTile = document.createDocumentFragment('<div class="antTile" id="ant' + id + '"><div class="energy_container"><div class="energy"></div></div><div class="avatar head' + head + '"></div><div class="name">' + name + '</div><div class="distance">0</div><div class="delivered">0 delivered</div></div>');
        
		document.querySelector("#tilesDrawer").appendChild(antTile);
		antTile.addEventListener('click',function(evt) {
			document.querySelector('.antTile').forEach(function() {
		...
		return {
			...
			pickupFood: function() {
				this.hasFood = true;
				addClass(this.tile, 'hasFood');
			},
			dropFood: function() {
				this.hasFood = false;
				removeClass(this.tile,'hasFood');
				this.delivered ++;
			},
			die: function(index) {
				if(this.alive)	{
					...
					addClass('#ant' + this.id + ' .name', 'dead'); 
					addClass('#ant' + this.id + ' .avatar', 'dead');
				}
			},
			update: function() {
				var units = 'cm';
				...
				this.tile.childNodes['.distance'] = theDist.toFixed(2) + units;
				this.tile.childNodes['.delivered'] = this.delivered + " delivered";
			}
		}
	};
	...
	antButton.addEventListener('click', function(evt) {
		removeClass(cf, 'circle_cursor');
		removeClass(cf, 'hand_cursor');
		var d = document.querySelectorAll('.panel button');
        var i;
        for (i = 0; i < d.length; ++i) {
          removeClass(d[i], 'active');
        }
	...
	pauseButton.addEventListener('click', function(evt) {
		paused = !paused;
		toggleClass(this, 'pauseIcon');
		toggleClass(this, 'playIcon');
		console.log(paused);
	...
	foodButton.addEventListener('click', function(evt) {
		removeClass(cf, 'circle_cursor');
		removeClass(cf, 'hand_cursor');
        var d = document.querySelectorAll('.drawMode button');
        var i;
        for (i = 0; i < d.length; ++i) {
          removeClass(d[i], 'active');
        }
	...
	document.querySelector('#eraserButton').addEventListener('click', function(evt) {
		removeClass(cf, 'hand_cursor');
		var d = document.querySelectorAll('.drawMode button');
        var i;
        for (i = 0; i < d.length; ++i) {
	...
	fadeIn(document.querySelector('#hill')); anim();
	...
	if(!paused){
		...
		for(var ant = 0; ant < numAnts; ant++) {				
			if(ants[ant] && ants[ant].alive && ants[ant].delay == 0 ) {
				...
				ctxo.fillRect(ants[ant].x, ants[ant].y,4,4);
				ctxo.fillRect(ants[ant].l.x, ants[ant].l.y,1.3,1.3);
				ctxo.fillRect(ants[ant].r.x, ants[ant].r.y,1.3,1.3);
				ctxo.fillRect(ants[ant].l2.x, ants[ant].l2.y,1.3,1.3);
				ctxo.fillRect(ants[ant].r2.x, ants[ant].r2.y,1.3,1.3);
				if(ants[ant].hasFood) {
					ctxo.fillStyle = "#303F9F";
					ctxo.fillRect(ants[ant].x, ants[ant].y,4,4);
				}
			}
		}
